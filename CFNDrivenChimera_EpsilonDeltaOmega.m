function [CMF, S, degreeSync, decayTime] = CFNDrivenChimera_EpsilonDeltaOmega(N, b, alphaD, nEpsilon, nDOmega, ii)

% Simulate a 1-layer chimera network driven by an OU-perturbed phase
...
% considered in the paper (in this case, 100; that is ii=1:100).
...
%              for each (epsilon, DeltaOmega). Also known as entrainment
%              power in the paper.
...
%              (R > 0.9999) for each (epsilon, DeltaOmega). It is a useful
%              information to compute the collapse power which is the
%              number of collapsed realizations (so the times decayTime is less
%              than 1000 divided for the number of total realizations
%              considered, in the case of the paper it is 100).
...
observationTime  = 1000;              % duration used for evaluating the OU driver
...
%% OU process parameters (for the driver)
tau   = 223;
sigma = 0.5;
mu    = 0;
...
I = (observationStart/dt + 1):1:integrationSamples;
...
for i = 1:nDOmega*nEpsilon
    ...
    %% Generate the external driving signal (OU noise on top of a linear trend)
    Omega  = -0.5;  % mean frequency of the driver (mean slope)

    % Simulate OU process longer than needed to discard initial transient
    driver = simulate_ou_process(1 / tau, mu, sigma, 0, ...
                                 observationTime + 5 * tau, dt);

    % Discard first 5*tau to ensure stationarity of the OU process
    start_idx = round(5 * tau / dt) + 1;
    driver    = driver(start_idx:end);

    % Keep exactly observationTime/dt samples
    driver = driver(1:round(observationTime / dt));

    % Add linear trend Omega*t to the OU fluctuations
    time_vector = dt:dt:observationTime;
    driver      = Omega * time_vector + driver;
